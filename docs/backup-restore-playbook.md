...
- Nombre del archivo:
  - `inventario_<rows>rows_YYYY-MM-DD_HH-MM-SS.dump` por defecto,
  - `<prefijo>_<rows>rows_YYYY-MM-DD_HH-MM-SS.dump` si pasas un prefijo manual.
- Requisito:
...
- Conteo:
  - el valor `<rows>` es la suma exacta de filas en todas las tablas `public` del esquema, excluyendo `_prisma_migrations`.
- Retencion:
  - 30 dias por defecto.
